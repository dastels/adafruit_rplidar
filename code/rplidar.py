...
def _process_scan(raw):
    ...
    new_scan = bool(int(raw[0]) & 0b1)
    inversed_new_scan = bool((int(raw[0]) >> 1) & 0b1)
    quality = int(raw[0]) >> 2
    ...
    angle = ((int(raw[1]) >> 1) + (int(raw[2]) << 7)) / 64.
    distance = (int(raw[3]) + (int(raw[4]) << 8)) / 4.
...
class RPLidar(object):
    ...
    _serial_port = None  #: serial port connection.. a UART object
    ...
    def _read_descriptor(self):
        ...
        is_single = int(descriptor[-2]) == 0
        return int(descriptor[2]), is_single, int(descriptor[-1])
    ...
    def get_health(self):
        ...
        if dtype != HEALTH_TYPE:
            raise RPLidarException('Wrong response data type')
        raw = self._read_response(dsize)
        status = _HEALTH_STATUSES[int(raw[0])]
        error_code = (int(raw[1]) << 8) + int(raw[2])
